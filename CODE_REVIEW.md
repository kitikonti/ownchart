...
- [x] `src/components/GanttChart/GridLines.tsx` (267 LOC) — Hex-Werte waren bereits in GRID-Tokens. Opacity magic numbers→GRID.weekendOpacity/holidayOpacity, Grid-Schwellwerte→benannte Konstanten, endDate-Berechnung dedupliziert (3×→shared useMemo), holidayService.setRegion() aus useMemo verschoben, getStroke→Top-Level-Helper. **Review-complete — 0 offene Findings.**
