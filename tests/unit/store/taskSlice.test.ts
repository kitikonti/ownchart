 ...
 * insertion/ordering tests (progress=0, project default color, empty metadata).
 */
const makeTask = (
  fields: Omit<Task, 'progress' | 'color' | 'metadata'>
): Task => ({
  ...fields,
  progress: 0,
  ...
  describe('Indent/Outdent Operations', () => {
    describe('canIndentSelection / canOutdentSelection', () => {
      ...
      it('should return true when can outdent', () => {
        const { toggleTaskSelection, canOutdentSelection } =
          useTaskStore.getState();

        // Make task-2 a child of task-1 via direct state update
        const tasks = useTaskStore.getState().tasks.map((t) =>
          t.id === 'task-2' ? { ...t, parent: 'task-1' } : t
        );
        useTaskStore.setState({ tasks });
        toggleTaskSelection('task-2');
